...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(metalKitView: MTKView, _ mIdent:Int) {
        ident = mIdent
        guard let queue = device3D.makeCommandQueue() else { return nil }
        self.commandQueue = queue
        
        let uniformBufferSize = alignedUniformsSize * maxBuffersInFlight
        
        guard let buffer = device3D.makeBuffer(length:uniformBufferSize, options:[MTLResourceOptions.storageModeShared]) else { return nil }
        ...
        do {
            pipelineState = try Renderer.buildRenderPipelineWithDevice(device: device3D,
        ...
        guard let state = device3D.makeDepthStencilState(descriptor:depthStateDesciptor) else { return nil }
        depthState = state
